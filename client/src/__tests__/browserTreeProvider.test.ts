...
describe('BrowserTreeProvider', () => {
  ...
  describe('with active session (maxEnvironment=0)', () => {
    ...
    it('returns class category nodes under a dictionary with ** ALL ** first and ** OTHER GLOBALS ** last', async () => {
      ...
      expect(children[0]).toMatchObject({ kind: 'classCategory', name: '** ALL **', dictName: 'Globals' });
    ...
    it('returns all classes sorted under ** ALL ** class category', async () => {
      const dict: BrowserNode = { kind: 'dictionary', sessionId: 1, dictIndex: 1, name: 'Globals' };
      await provider.getChildren(dict);

      const allCat: BrowserNode = {
        kind: 'classCategory', sessionId: 1, dictIndex: 1, dictName: 'Globals', name: '** ALL **',
      };
    ...
    it('returns method category nodes under a side with ** ALL ** first', async () => {
      ...
      expect(children).toHaveLength(3);
      expect(children[0]).toMatchObject({ kind: 'category', name: '** ALL **', environmentId: 0 });
    ...
    it('returns all methods sorted alphabetically under ** ALL **', async () => {
      const cat: BrowserNode = {
        kind: 'category', sessionId: 1, dictIndex: 1, dictName: 'Globals',
        className: 'Array', isMeta: false, name: '** ALL **', environmentId: 0,
      };
  ...
  describe('with active session (maxEnvironment=2)', () => {
    ...
    it('returns categories from env cache for a specific environment', async () => {
      ...
      const children = await provider.getChildren(side);
      expect(children).toHaveLength(2);
      expect(children[0]).toMatchObject({ kind: 'category', name: '** ALL **', environmentId: 1 });
    ...
    it('returns only ** ALL ** for an environment with no methods', async () => {
      const side: BrowserNode = {
        kind: 'side', sessionId: 1, dictIndex: 1, dictName: 'Globals', className: 'Array',
      ...
      expect(children[0]).toMatchObject({ kind: 'category', name: '** ALL **' });
    });

    it('returns all methods from ** ALL ** for env 1', async () => {
      const cat: BrowserNode = {
        kind: 'category', sessionId: 1, dictIndex: 1, dictName: 'Globals', className: 'Array',
        isMeta: false, name: '** ALL **', environmentId: 1,
      };
      const children = await provider.getChildren(cat);
      expect(children).toHaveLength(2);
    ...
    it('returns empty methods from ** ALL ** for env with no methods', async () => {
      const cat: BrowserNode = {
        kind: 'category', sessionId: 1, dictIndex: 1, dictName: 'Globals', className: 'Array',
        isMeta: true, name: '** ALL **', environmentId: 2,
      };
      const children = await provider.getChildren(cat);
  ...
  describe('getTreeItem', () => {
    ...
    it('renders a class category node', () => {
